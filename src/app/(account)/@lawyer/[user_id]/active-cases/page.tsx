...
const Sidebar: React.FC<{
  ...
  >
    <nav className="flex flex-col mt-3 w-full text-xs font-medium">
        ...
        <button 
          onClick={() => setSelectedCaseType("ongoing")}
          className={`flex gap-2 mt-1 items-center hover:underline ${
            selectedCaseType === "ongoing" ? "text-[#0838E5] font-bold" : "text-black"
          }`}
        >
          <img
            ...
            aria-hidden="true"
          />
          <span className="font-bold">Ongoing Cases</span>
          {selectedCaseType === "ongoing" && <IoMdCheckmark className="ml-auto text-blue-600 text-xl" />}
        </button>
        <button 
...
const InputDesign: React.FC = () => {
  ...
  const userId = params?.user_id as string;
    ...
    console.log("Dynamic route user_id:", userId);
  ...
  const filteredCases = cases.filter((caseItem) => {
    ...
    // ✅ Return only if assigned to user and other filters match
