...
export default function AcceptCasePage() {
  ...
  useEffect(() => {
    async function loadCase() {
      console.log("Effect triggered for caseId:", caseId, "(type:", typeof caseId, ")"); // Log ID from URL
      ...
      if (response.success && response.data) {
        ...
        const foundCase = response.data.find((c: Case) => {
          console.log(`Comparing URL ID "${caseId}" with Case ID ${c.id} (type: ${typeof c.id})`);
          // Convert API ID to string for comparison OR URL ID to number
          return String(c.id) === caseId; // Safest: Compare as strings
          // OR return c.id === parseInt(caseId, 10); // If you know c.id is always a number
        });
        ...
        if (foundCase) {
          ...
          console.error(`Case with ID ${caseId} not found in the fetched list.`);
  ...
  const handleAcceptCase = async () => {
    ...
    if (response.success) {
      ...
      router.push("/dashboard/case"); // Navigate after success
    } else {
