...
export default function AcceptCasePage() {
  ...
  useEffect(() => {
    async function loadCase() {
      ...
      console.log("Case ID from params:", caseId); // ✅ log caseId
  
      if (response.success && response.data) {
        const foundCase = response.data.find((c) => String(c.id) === String(caseId));
        console.log("Found case:", foundCase); // ✅ log matched case
  
        if (foundCase) {
          setCaseData({
            ...foundCase,
            media: Array(5).fill("/defaultphoto.jpg"),
            files: ["witness_report.pdf", "suspect_profile.docx", "evidence.zip"],
          });
        }
      }
            ...
            <div className="md:col-span-2 h-[65vh] flex flex-col rounded-xl pr-15 pt-15 overflow-hidden">
              ...
              <div className="shrink-0 mt-0 mb-8 ml-8 flex justify-between items-center pr-10">
                <BaseButton
                  ...
                  textColor="white"
                  onClick={() => {
                    alert("Case deleted (mock)");
                    router.push("/dashboard/active-cases");
                  }}
                >
                  Delete Case
                </BaseButton>
