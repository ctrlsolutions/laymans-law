"use client";

import { FC, useMemo } from "react";
import { FormSelectProps } from "@/interface/ComponentTypes";
import { iconMapping } from "@/utils/IconMapping";

const BaseFormSelect: FC<FormSelectProps> = ({
  label,
  name,
  color = "black",
  width,
  height,
  textSize,
  value,
  choices,
  onChange,
}) => {
  return (
    <label
      htmlFor={name}
      className={`flex flex-col space-y-1 mt-4 text-${color}`}
    >
      <span className={`font-extrabold ${textSize || 'text-sm'} sm:text-sm md:text-base lg:text-lg xl:text-xl`}>
        {label}
      </span>

      <div className={`relative flex items-center ${width || ''}`} style={{ width, height }}>
        <select
          id={name}
          name={name}
          value={value}
          onChange={onChange}
          className={`text-${color} p-2 w-full text-ellipsis 
            border border-gray-300 font-semibold 
            focus:outline-none focus:ring-0 rounded-[0.375rem] 
            ${textSize || 'text-sm'}`}
          style={{ height }}
        >
          {choices.map((option) => (
            <option key={option.value} value={option.value}>
              {option.label}
            </option>
          ))}
        </select>
      </div>
    </label>
  );
};

export default BaseFormSelect;
